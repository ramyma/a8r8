defmodule ExSd.Sd.ComfyPrompt4 do
  require Logger
  ...
  def txt2img(%GenerationParams{} = generation_params, attrs) do
    ...
    prompt =
      ...
      |> maybe_add_ip_adapters(attrs, is_txt2img: generation_params.txt2img)
  ...
  @spec maybe_add_controlnet(prompt(), list(), GenerationParams.t(), any(), boolean()) :: prompt()
  ...
  @spec maybe_add_regional_prompts_with_coupling(prompt(), map(), [
          ...
          | {:width, non_neg_integer()}
          | {:height, non_neg_integer() | {:is_txt2img, boolean()}}
        ]) :: prompt()
  ...
  @spec maybe_add_ip_adapters(prompt(), map(), [{:is_txt2img, boolean()}]) :: prompt()
      ...
      |> add_ip_adapter_unified_loader("unified_ip_adapter_loader_#{preset}",
        model:
          if(Enum.empty?(positive_loras),
            do:
              if(is_txt2img,
                do: get_base_model(true),
                else:
                  if(fooocus_inpaint,
                    do: node_ref("apply_fooocus_inpaint", 0),
                    else: get_base_model(is_txt2img)
                  )
              ),
            else: node_ref("positive_lora#{length(positive_loras) - 1}", 0)
          ),
