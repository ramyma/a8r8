...
const LoraDetails = ({ name, metadata }: Props) => {
  ...
  console.log(imagesContRef.current?.getBoundingClientRect()?.width ?? 0);
    ...
    <div className="relative flex content-around gap-5 h-full">
      ...
      <div className="flex flex-col flex-1 w-[25%] gap-5">
        ...
        <div className="flex flex-col gap-2 items-start">
          ...
          <div className="flex flex-wrap gap-2">
            {responseLora?.trainedWords?.map((word, index) => (
        ...
        <Button
          ...
          disabled={isLoading || !metadata?.sshs_model_hash}
        >
          Fetch from Civit
        </Button>
      ...
      <ScrollArea className="flex-[5]">
        ...
        >
          {transitions((style, image, _, index) => (
            <animated.div
            ...
            >
              ...
              {image.isActive && (
                <div className="flex">
                  <Button
                    onClick={() =>
                      activeImage > 0 && setActiveImage((prev) => prev - 1)
                    }
              ...
              {image.isActive && image.meta?.negativePrompt && (
                <div className="px-2 py-4 text-left flex-col items-start flex place-items-center gap-1">
